...
export default function AnimatedNavigationTabs({ items }: { items: Props[] }) {
  const [active, setActive] = useState<Props>(items && items.length > 0 ? items[0] : {id: 1, tile: ""});
        ...
        <ul className="flex items-center justify-center">
          {items.map((item) => (
            ...
            >
              ...
              {active.id === item.id && (
                <motion.div layoutId="active" className="absolute bottom-0 left-0 right-0 w-full h-0.5 bg-primary" />
              )}
              {isHover?.id === item.id && (
                <motion.div
                  layoutId="hover"
